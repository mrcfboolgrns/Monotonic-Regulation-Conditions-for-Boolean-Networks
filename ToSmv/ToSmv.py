...
class ToSmv:
    ...
    def run_nuxmv(self,input_file,reg): #run the newXmv program with a batch file and get the result
        ...
        if result.stdout.count("true") == len(self.BoolNet.experiments): #if the count of experiments that were true is the same as number of experiments in general then
            #success
            return True
        return False
    ...
    def run_nuxmvMAC(self, input_file, reg):
        ...
        if result.stdout.count("true") == len(self.BoolNet.experiments):
            return True  # Suc

    # def all_combined1(self):
    #     self.all_regulation_conditions()
    #     self.BoolNet.add_perm()
    #
    ...
    #             try:
    #                 reg = reg_queue.get_nowait()
    #                 self.python_to_nuxmv()
    #                 reg_queue.task_done()
    #             except queue.Empty:
    #                 break
    #
    ...
    def all_combined(self):
        ...
        self.all_regulation_conditions() #get all regulation permutations
        j=0
        i=0
        # self.BoolNet.add_perm()
        # print(f"fdbjskaadfbjkdfasdfa{self.BoolNet.perm_interactions}")
        while self.BoolNet.perm_index < len(self.BoolNet.perm_interactions): #iterate over all permultations
            i=0
            flag = True

            l = self.BoolNet.add_perm()
            while i<len(self.reglist): #iterate over each regulation and write it to smv
                res = self.python_to_nuxmv(i)
                if res and flag:
                    permutations_h.write(str(l) + "\n")
                    flag = False #only present one solution even when more reg conditions work

                if i == self.num_of_perms: #set how many permutations you want for each network
                    break  ####################change back regulation break
                else:
                    i += 1
                # i+=1
            #break
        ...
        #     # self.python_to_nuxmv()
        #
        #     while self.BoolNet.perm_index < len(self.BoolNet.perm_interactions):
        #         l = self.BoolNet.add_perm()
        #
    ...
    # def threading(self, maxThreads=5):
    #     threads = [self.python_to_nuxmv.start() for k in self.reglist[:maxThreads]]
    #     for t in threads:
    #         t.join()
    #         ret = t.result

    def python_to_nuxmv(self,regidx): #from python to an smv file
        filename = "model.smv"
        ...
        with open(filename, "w") as f:
            ...
            for k in range(n):
                ...
                for i in self.BoolNet.components.keys():
                    ...
                    str1+=f"    next({i}{k}) := (time{k} < {self.BoolNet.experiments[k][1][0]} ? {dict1[i+str(k)][reg[j]]}:{i}{k});\n"#define the next for each component in exp
                    j+=1
            for exp in self.BoolNet.experiments:
